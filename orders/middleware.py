...
class MiddlewarePermissions(MiddlewareMixin):
    ...
    def __call__(self, request):
        ...
        if app_name_space == [app_name]:
            ...
            user = get_object_or_404(CustomUser, pk=pk, id=request.user.id, status=ACTIVE_USER)

            if any([not user == request.user, not user.groups.filter(name=CLIENT_GROUP).exists()]):
